...
description: >-
  Read-only gatherer for ainotes-report. Given a date window and scope, scans the notes repo (notes, plans, tasks, daily files, INDEX.md, PRS.md, TASKS.md) via frontmatter and ledger rows and returns one compact JSON document — one item per in-window file with its PRs (plus ledger status), Jira keys, tasks, a one-line outcome and any Cost figures, plus counts and the earliest recorded date. Never writes, never calls GitHub or Jira, never invents items that aren't backed by a file or ledger row.
...
(inclusive); `scope` is `workspace`, `repo:<name>` or `epic:<key>`. All data is directly under
`<notes_repo_path>/`.

## What to scan

1. `notes/*.md`, `plans/*.md` — filter by frontmatter `date` (Grep `^date:` across the folder
...
2. `tasks/*.md` + `TASKS.md` — tasks whose `created` is in the window, or whose ledger
...
3. `daily/*.md` in the window — count them; include one item each only if it names a PR/Jira/task.
4. `INDEX.md` — catch in-window files tagged with the scope's epic/repo that the date scan missed.
5. `PRS.md` — for every PR URL/number mentioned in an item, its current section
...
7. `earliest_recorded_date`: the minimum frontmatter `date` across all of `notes/` and `plans/`
   (same single Grep).
...
- Paths are relative to the repo root. `files` lists every file you read, so the caller can spot-check.
...
- Unknown fields are `null`/`[]`, never guessed. If the notes repo still has a `db/` subfolder
  (pre-flattening legacy layout), return `{"error": "legacy layout"}`.
